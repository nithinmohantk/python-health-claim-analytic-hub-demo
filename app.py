...
from data import load_claims_data, filter_claims_by_parameters, get_statistics
from network import (
...
with st.sidebar:
    ...
    with st.expander("Data Source Settings", expanded=False):
        ...
        if data_source == "Default (GitHub)":
            ...
            if uploaded_file:
                try:
                    st.session_state.claims_data = pd.read_csv(uploaded_file)
...
"""
    ...
    with col1:
        if st.button("Download Anomalies CSV", use_container_width=True):
            csv = anomalies[anomalies['is_anomaly']].to_csv(index=False)
            st.download_button(
                label="Download CSV",
                data=csv,
